            ...
            switch (Player_GetMask(globalCtx)) {
                case PLAYER_MASK_SKULL:
                    if (!Flags_GetTreasure(globalCtx, 0x1F)) {
                        ...
                        GiveItemWithoutActor(globalCtx, GetRandomizedItemIdFromKnownCheck(check, GI_STICK_UPGRADE_30), check);
                ...
                case PLAYER_MASK_TRUTH:
                    if (!Flags_GetTreasure(globalCtx, 0x1E)) {
                        ...
                        GiveItemWithoutActor(globalCtx, GetRandomizedItemIdFromKnownCheck(check, GI_NUT_UPGRADE_40), check);
