...
void func_80AA0EA0(EnMa1* this, GlobalContext* globalCtx) {
    if (Actor_HasParent(&this->actor, globalCtx)) {
        ...
        RandomizerCheck check = RC_HC_MALON_EGG;
        if (gSaveContext.n64ddFlag) {
            GetItemID getItemId = GetRandomizedItemIdFromKnownCheck(check, GI_WEIRD_EGG);
            func_8002F434(&this->actor, globalCtx, getItemId, 120.0f, 10.0f, check);
        } else {
            func_8002F434(&this->actor, globalCtx, GI_WEIRD_EGG, 120.0f, 10.0f, check);
        }
    }
}
...
void GivePlayerRandoRewardMalon(EnMa1* malon, GlobalContext* globalCtx, RandomizerCheck check) {
    ...
    if (malon->actor.parent != NULL && malon->actor.parent->id == GET_PLAYER(globalCtx)->actor.id &&
        ...
        func_8002F434(&malon->actor, globalCtx, getItemId, 10000.0f, 100.0f, check);
    }
}
