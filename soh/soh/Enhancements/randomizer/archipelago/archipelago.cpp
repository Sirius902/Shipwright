...
static std::shared_ptr<Archipelago> archipelagoInstance;

static const std::string& getCheckName(RandomizerCheck check) {
    return ranges::find_if(
        SpoilerfileCheckNameToEnum,
        [&](const auto& pair) { return pair.second == check; }
    )->first;
}

std::string Archipelago::getItemText() {
...
void Archipelago::setCurrentCheck(RandomizerCheck check, GetItemID getItemId) {
    if (std::abs(getItemId) == GI_ARCHIPELAGO_ITEM) {
        currentCheck = check;
    }
}

void Archipelago::obtainCheck() {
    if (!currentCheck) {
    ...
    SPDLOG_INFO("Archipelago Check Obtained ({})", getCheckName(check).c_str());

    currentCheck = {};
}

std::shared_ptr<Archipelago> Archipelago::getInstance() {
...
static void EmitSettings(YAML::Emitter& out, const std::unordered_map<RandomizerSettingKey, u8>& cvarSettings) {
    for (const auto& [k, v] : cvarSettings) {
        // Skip because these settings are not implemented yet.
        if (k == RSK_RANDOM_TRIALS || k == RSK_SHUFFLE_CHEST_MINIGAME) {
            continue;
        }
...
void Archipelago::generateConfig(const std::unordered_map<RandomizerSettingKey, u8>& cvarSettings) {
    YAML::Emitter out;

    const char* game = "Ship of Harkinian";

    std::string playerName = CVar_GetString("gArchipelagoName", "");
    // ARCHIPELAGO_TODO: Find out if any extra validation needs to be done.
    if (playerName.empty()) {
        throw std::logic_error{"Player Name must be set"};
    } else if (playerName.size() > nameMaxLength) {
        throw std::logic_error{"Player Name is too long, must be at most " +
            std::to_string(nameMaxLength) + " characters"};
    }
    ...
    out << YAML::Key << "description" << YAML::Value << YAML::SingleQuoted << "Generated by https://github.com/Sirius902/Shipwright/tree/archipelago";
    out << YAML::Key << "game" << YAML::Value << game;
...
static void DrawArchipelagoWindow(bool& open) {
    // ARCHIPELAGO_TODO: Add Archipelago window.
}

void Archipelago_Init() {
    SohImGui::AddWindow("Randomizer", "Archipelago", DrawArchipelagoWindow);
    // ARCHIPELAGO_TODO: Only initialize instance when connected to server.
    archipelagoInstance = std::make_shared<Archipelago>();
}
